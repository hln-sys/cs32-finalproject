...
from hued import hex_to_rgb  # use Hued library to convert hex strings to RGB tuples
...
def show_color_slider():
    ...
    while True:
        ...
        for x in range(hue_rect.width):
            color = hsv_to_rgb(x / hue_rect.width, 1, 1)
        ...
        for x in range(sat_rect.width):
            color = hsv_to_rgb(hue, x / sat_rect.width, value)
        ...
        for x in range(val_rect.width):
            color = hsv_to_rgb(hue, saturation, x / val_rect.width)
